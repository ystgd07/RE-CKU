...
export const createResumeQ = async (userId: number, newResumeName: string) => {
  const newResume = await db.query(`INSERT INTO resume(usedUserId, name)
                                    VALUES (?, ?)`, [userId, newResumeName]);
...
export const findMyResumesQ = async (userId: number) => {
  const [myResumes,] = await db.query(`SELECT id          AS resumeId,
                                              name        AS resumeName,
...
export const findCareersQ = async (resumeId: number) => {
  const [careers,] = await db.query(`SELECT id           AS careerId,
                                            company,
                                            position,
                                            notDevlop,
                                            workNow,
...
export const findProjectsQ = async (resumeId: number) => {
  const [projects,] = await db.query(`SELECT id AS projectId,
...
export const findCareerQ = async (careerId: number) => {
  const [career,] = await db.query(`SELECT id           AS careerId,
                                           company,
                                           position,
                                           notDevlop,
                                           workNow,
...
export const updateResumeQ = async (resumeId: number, updateResumeInfo: Record<string, string | number>) => {
  ...
  const updatedResume = await db.query(`UPDATE resume
                                        SET ${key.join(", ")}
                                        WHERE id = ?`, [...value, resumeId]);
...
export const updateCareerQ = async (careerId: number, updateCareerInfo: Record<string, string | number>) => {
  ...
  const updatedCareer = await db.query(`UPDATE career
                                        SET ${key.join(", ")}
                                        WHERE id = ?`, [...value, careerId]);
...
export const updateProjectQ = async (projectId: number, updateProjectInfo: Record<string, string | number>) => {
  ...
  const updatedProject = await db.query(`UPDATE project
                                         SET ${key.join(", ")}
...
export const deleteCareerQ = async (careerId: number) => {
  const deletedCareer = await db.query(`DELETE
                                        FROM career
                                        WHERE id = ?`, careerId);
...
export const deleteProjectQ = async (projectId: number) => {
  const deletedProject = await db.query(`DELETE
                                         FROM project
                                         WHERE id = ?`, projectId);
